"""
実ニュース(news-bot由来)とTA期待度の整合性係数を算出するモジュール

news-bot が GitHub raw に書き出す news_state.json / reports_state.json を
analyzer が取得し、銘柄別に集約して整合性係数を算出する。

このモジュールは決定論（LLM不介在）。入力は NewsItem のリストと TA 方向のみ。
（旧: Gemini方向を受けるスカラ版 calc_alignment_coefficient が存在したが、
  LLMを数値経路から排除するリファクタで除去済み）
"""
